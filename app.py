...
def ai_complete(prompt: str) -> str:
    ...
    if USE_GEMINI:
        try:
            ...
            return clean_markdown((resp.text or "").strip() or "(empty AI response)")
        except Exception as e:
            return f"(AI error: {e})"
    
    
    if "explain" in prompt.lower():
        return clean_markdown("explanation (offline): break the topic into definitions, key formulas, and 2–3 solved examples.")
    
    if "quiz" in prompt.lower():
        return clean_markdown("1) Q: Define inertia. A: Resistance to change in motion.\n2) Q: State Ohm’s law. A: V=IR.")
    
    return clean_markdown("offline mode: no API key set; add details and try again.")
